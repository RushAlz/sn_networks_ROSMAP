...
fea_output = "/pastel/Github_scripts/SpeakEasy_dlpfc/bulk_dlpfc/FEA_analysis/gprofiler/"
...
for(i in 1:length(clusters_list)){
  ...
  gostres <- gost(query = module$symbol, organism = "hsapiens", correction_method = "gSCS", significant = TRUE, user_threshold = 0.05)
  if (!is.null(gostres)){
...
#Save for downstream
save(output_df, file = paste0(fea_output, "gprofiler_MF.Rdata"))
```
